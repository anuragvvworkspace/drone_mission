...
class midterm_drone():
	def __init__(self):
		...
		while(not rospy.is_shutdown()):
			if((rospy.Time.now() - sec_req) > rospy.Duration(1.0)):
				...
				sec_req = rospy.Time.now()
			if(self.current_state.mode != "OFFBOARD" and (rospy.Time.now() - last_req) > rospy.Duration(3.0)):
				if(set_mode_client.call(offb_set_mode).mode_sent == True):
					rospy.loginfo("OFFBOARD enabled")
				
				last_req = rospy.Time.now()
			else:
				if(not self.current_state.armed and (rospy.Time.now() - last_req) > rospy.Duration(3.0)):
		...
		self.rock_x,self.rock_y = 60.2,-12.5
		#rospy.spin();
		print("------------================= READY to GO =============-----------------")
	...
	def navigate(self, x, y, z, ):
		...
		while d > 1.0 and not rospy.is_shutdown():
			# print("Drone pose received", self.curr)
			# self.curr = self.currPose.pose.position
	...
	def gotoRock(self):
		print("=== GO to rock and Face rock ========")
		locations = numpy.matrix([[60, -15, 18.89],[57, -12, 18.89],[60, -15, 18.89],[63, -12, 18.89],[60, -9, 18.89],[57, -12, 18.89]])
		for waypt in locations:
			x, y, z = waypt.tolist()[0]
			self.navigate(x, y, z)
